...
void startSpiffs() {
	...
	//SPI.begin();
	if (!SPIFFS.begin()
		|| (!SPIFFS.exists(WIFICONF) && !SPIFFS.exists(DBCONF) && !SPIFFS.exists(KARBOCONF) && !SPIFFS.exists(SPUNDERCONF)))
	{
	...
	if (SPIFFS.begin())
	{
		...
		if (SPIFFS.exists(configfile))
		{
			...
			if (json.containsKey("SSID"))
				WIFIUSR = (const char)json["SSID"];
			if (json.containsKey("PSK"))
				WIFIPASS = (const char)json["PSK"];
		}
		else {
...
bool readKarboConfigFile(float &SKARBO, ulong &MINTERVAL, float &KARBOHYST) {
...
bool writeKarboConfigFile(float &SKARBO, ulong &MINTERVAL, float &KARBOHYST) {
	String configfile = KARBOCONF;
	SerialOut(F("writeConfigFile "), false);
	SerialOut(configfile);
	if (!((configfile == WIFICONF) || (configfile == DBCONF) || (configfile == KARBOCONF) || (configfile == SPUNDERCONF))) {
